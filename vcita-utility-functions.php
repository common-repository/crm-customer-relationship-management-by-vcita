...
function vcita_create_link($caption, $page, $params = "", $options = array()) {
	...
	$link = "https://".VCITA_SERVER_BASE."/".$page."?ref=".VCITA_WIDGET_API_KEY.$params_prefix.$params.$origin_prefix.$origin;
...
function vcita_parse_expert_data($raw_data) {
	...
	$vcita_widget = array('title' => filter_var(['title'], FILTER_SANITIZE_SPECIAL_CHARS),
						  'created' => '1',
						  'uid' => filter_var(['uid'], FILTER_SANITIZE_SPECIAL_CHARS),
						  'new_install' => 'false',
		    			  'email' => filter_var($raw_data['email'], FILTER_SANITIZE_EMAIL),
						  ...
						  'engage_active' => 'true',
						  'confirmation_token' => filter_var(['confirmation_token'], FILTER_SANITIZE_ENCODED),
						  'implementation_key' => filter_var(['implementation_key'], FILTER_SANITIZE_STRING),
						  'confirmed' => filter_var($raw_data['confirmed']), FILTER_SANITIZE_STRING);
...
/**
 * Perform an https GET Call to retrieve the data for the required content.
 * 
 * @param $url
...
/**
 * Perform an https POST Call to retrieve the data for the required content.
 *
 * @param $url
 * @return array - raw_data and a success flag
 */
...
/**
 * Parse the https response and return the data and if was successful or not.
 */
...
function generate_or_validate_user($widget_params) {
    extract(vcita_post_contents("https://".VCITA_SERVER_BASE."/api/experts?".
